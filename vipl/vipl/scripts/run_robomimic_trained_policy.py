...
from robomimic.envs.wrappers import RandomizedCameraWrapper2
...
def rollout(policy, env, horizon, render=False, video_writer=None, video_skip=5, return_obs=False, camera_names=None):
    ...
    try:
        for step_i in range(horizon):
            ...
            if done or success:
                # append final freeze frame to match the full horizon length
                # (this is useful for video visualization)
                for _ in range(step_i, horizon):
                    if video_writer is not None:
                        if video_count % video_skip == 0:
                            video_img = []
                            for cam_name in camera_names:
...
def run_trained_agent(args):
    ...
    if args.randomize_camera:
        camera_pose_sampler = CameraPoseSampler(sampler_type=args.sampler_type)
        env = RandomizedCameraWrapper2(
            env,
            camera_name=args.camera_name,
            camera_pose_sampler=camera_pose_sampler,
            randomize_freq=args.samples_per_view,
        )

    # maybe create video writer
    video_writer = None
    if write_video:
        video_writer = imageio.get_writer(args.video_path, fps=20, mode="I", loop=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sampler_type",
        type=str,
        help="type of camera randomization to use",
        default="small_perturb"
    )
