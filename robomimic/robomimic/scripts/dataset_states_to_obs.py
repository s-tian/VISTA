...
def dataset_states_to_obs(args):
    ...
    # load all mask filter keys from the input file
    new_filter_keys = dict()
    if "mask" in f:
        for k in f["mask"].keys():
            new_filter_keys[k] = np.array([], dtype='|S8')
    ...
    for parse_iter in tqdm(range(args.parse_iters)):
        for ind in tqdm(range(len(demos))):
            ...
            output_ind = parse_iter * len(demos) + ind
            ...
            # update filter keys
            for k in new_filter_keys:
                if ep.encode("utf-8") in f["mask"][k]:
                    # np append creates a copy of the array, which is inefficient but fine for this script
                    new_filter_keys[k] = np.append(new_filter_keys[k], output_ep_name.encode("utf-8"))
    ...
    if "mask" in f:
        ...
        for k in new_filter_keys:
            f_out["mask"].create_dataset(k, data=new_filter_keys[k])
