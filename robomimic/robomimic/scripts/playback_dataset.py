...
def playback_trajectory_with_obs(
    ...
    for i in range(traj_len):
        if video_count % video_skip == 0:
            ...
            # check the dtype of im, if it's bytes, we need to unjpeg it
            # unfortunately the bytes datatype has many dtypes, so we just consider anything not uint8 as jpeg bytes
            if im[0].dtype != "uint8":
                im = [read_image_from_bytes(b) for b in im]
