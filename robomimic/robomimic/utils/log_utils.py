...
class DataLogger(object):
    ...
    """
    def __init__(self, log_dir, config, log_tb=True, log_wandb=False, log_neptune=False):
        """
        Args:
        ...
        if log_wandb:
            ...
            for attempt in range(num_attempts):
                try:
                    ...
                    # Try to append the current date to the experiment name, for example if it's 11/27/2023, the experiment
                    # name will have 11_27_ in front of it. This is to prevent confusion from experiments with the same name.

                    experiment_name = config.experiment.name
                    date = datetime.now()
                    date_str = "{}_{}_".format(date.month, date.day)
                    experiment_name = date_str + experiment_name
        ...
        if log_neptune:
            import robomimic.macros as Macros
            os.environ["NEPTUNE_API_TOKEN"] = Macros.NEPTUNE_API_TOKEN
            os.environ["NEPTUNE_PROJECT"] = Macros.NEPTUNE_PROJECT
            ...
            # project and api token are inferred from environment variables
            self._neptune_logger = neptune.init_run(
                mode="async",
                name=experiment_name,
            )
            # set up info for identifying experiment
            neptune_config = {k: v for (k, v) in config.meta.items() if k not in ["hp_keys", "hp_values"]}
            # for (k, v) in zip(config.meta["hp_keys"], config.meta["hp_values"]):
            #     neptune_config[k] = v
            for (k, v) in config.items():
                neptune_config[k] = v
            if "algo" not in neptune_config:
                neptune_config["algo"] = config.algo_name
            if "slurm_id" not in neptune_config:
                neptune_config["slurm_id"] = os.environ.get("SLURM_JOB_ID", "local")
            self._neptune_logger["training"]["config"] = stringify_unsupported(neptune_config)
    ...
    def record(self, k, v, epoch, data_type='scalar', log_stats=False):
        ...
        if self._neptune_logger is not None:
            if data_type == 'scalar':
                self._neptune_logger["training"][k].append(v, step=epoch)
                if log_stats:
                    stats = self.get_stats(k)
                    for (stat_k, stat_v) in stats.items():
                        stat_k_name = '{}-{}'.format(k, stat_k)
                        self._neptune_logger["training"][stat_k_name].append(stat_v, step=epoch)
            # if data_type == 'video':
            #     self._neptune_logger["training"][f"{k}_{epoch}"].upload(v)
