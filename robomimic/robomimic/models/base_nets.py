...
class ZeroNVSConv(ConvBase):
    ...
    def __init__(
        ...
        super(ZeroNVSConv, self).__init__()
        self.pretrained_checkpoint = pretrained_checkpoint
        self.pretrained_config = pretrained_config
        self.input_channel = input_channel
        self.guidance_scale = guidance_scale
        self.scene_scale = scene_scale
        self.guidance = self._setup_zeronvs_model(self.pretrained_checkpoint, self.pretrained_config)
        self.feats_to_get = feats_to_get

        self.preprocess = nn.Sequential(
            transforms.Resize(256)
        )

        self.nets = self.get_features
    ...
    def output_shape(self, input_shape):
        ...
        return [32*len(self.feats_to_get), 32, 32]

    def get_features(self, inputs):
        inputs = self.preprocess(inputs)
        return self._get_intermediate_features(inputs)

    def _get_intermediate_features(self, input_image):
        batch_size = input_image.shape[0]
        c_crossattn, c_concat = self.guidance.get_img_embeds(
            input_image)
        target_camera = torch.from_numpy(np.eye(4)[None, ...]).repeat(batch_size, 1, 1).to(input_image.device).to(torch.float32)
        cond_camera = torch.from_numpy(np.eye(4)[None, ...]).repeat(batch_size, 1, 1).to(input_image.device).to(torch.float32)
        camera_batch = {
            "target_cam2world": target_camera,
            "cond_cam2world": cond_camera,
            "fov_deg": torch.from_numpy(np.array([45.0])).cuda().to(torch.float32).repeat(batch_size)
        }
        self.guidance.cfg.precomputed_scale = self.scene_scale
        ...
        latents = self.guidance.encode_images(input_image)
        x_in = torch.cat([latents] * 2)
        t = torch.tensor([997])
        t_in = torch.cat([t.reshape(1).repeat(batch_size)] * 2).to("cuda")
        xc = torch.cat([x_in] + c_concat, dim=1)
        cc = torch.cat(c_crossattn, 1)
        features = self.guidance.model.model.diffusion_model.forward_get_features(xc, t_in, context=cc)
        reduced_feats = [self.perform_pca(features[i]) for i in self.feats_to_get]        
        upscaled_features = []
        for feature in reduced_feats:
            if feature.shape[1] != 32 or feature.shape[2] != 32:
                # Upsample to 32x32
                upsampled = torch.nn.functional.interpolate(feature, size=(32, 32), mode='bilinear', align_corners=False)
                upscaled_features.append(upsampled)
            else:
                upscaled_features.append(feature)
        # Stack along the feature dimension
        stacked_features = torch.cat(upscaled_features, dim=1)
        return stacked_features


    def perform_pca(self, tensor, k=32):
        # tensor has shape (B, M, N) where B is batch size, M is the number of samples, N is number of features
        # output a tensor of shape (B, M, num_feats)
        # the batch dim here is actually a faked one, it's 2x the true batch size due to classifier free guidance
        # so we'll undo that first
        ...
        tensor_flat = tensor.reshape(B, N, -1)
        tensor_flat = tensor_flat.permute(0, 2, 1)
        # now shape is (B, H*W, N)
        U, S, V = torch.pca_lowrank(tensor_flat, q=k, center=True, niter=2)
        projected = torch.matmul(tensor_flat, V[:, :, :k])
        projected = projected.reshape(B, H, W, k)
        projected = projected.permute(0, 3, 1, 2)
        projected = projected / torch.max(projected)
        return projected
...
class MVMWMEncoder(ConvBase):

    def __init__(self, input_channels=3, checkpoint_path=None, freeze=True):
        super(MVMWMEncoder, self).__init__()
        try:
            from vipl.models.mvmwm_encoder import EncoderMultiViewVisionTransformer
        except ImportError:
            print("Couldn't find MV-MWM encoder from the VIPL repository")

        self.net = EncoderMultiViewVisionTransformer(
        ...
        # find key view_embed and decoder_view_embed and crop them from [1, 2, 1, 256] to [1, 1, 1, 256]
        state_dict['view_embed'] = state_dict['view_embed'][:, :1, :, :]
        state_dict['decoder_view_embed'] = state_dict['decoder_view_embed'][:, :1, :, :]
    ...
    def forward(self, inputs):
        # add dummy view dimension
        inputs = inputs[:, None] * 255.
        outputs = self.net(inputs)
        # remove dummy view dimension
        outputs = outputs[:, 0]
        return outputs
